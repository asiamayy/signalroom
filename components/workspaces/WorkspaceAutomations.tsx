...
import { BellRing, Loader2, Plus, Power, Trash2, Webhook } from 'lucide-react'
...
export function WorkspaceAutomations({ workspaceId }: { workspaceId: string }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const message = json.error || 'Could not load workflow notifications.'
        if (message.includes('workspace_automations') || message.includes('schema cache')) {
          setAvailable(false)
          setError('')
          return
        }
  ...
  return <section className="mt-10 rounded-[2rem] border p-6 sm:p-8" style={{ background: HOME_COLORS.surfaceContainerLowest, borderColor: HOME_COLORS.outlineVariant + '55' }}>
    <div className="flex flex-wrap items-start justify-between gap-5">
      <div><div className="flex items-center gap-2"><BellRing size={18} style={{ color: HOME_COLORS.primary }} /><h2 className="text-xl" style={{ fontFamily: HOME_FONT_DISPLAY, color: HOME_COLORS.primary }}>Workflow notifications</h2></div><p className="mt-2 max-w-2xl text-sm leading-relaxed" style={{ color: HOME_COLORS.onSurfaceVariant }}>Send an update to Slack or Microsoft Teams when shared research moves forward. Webhook addresses remain private and are never shown again.</p></div>
      {available && <button type="button" onClick={() => setOpen(value => !value)} className="inline-flex items-center gap-2 rounded-full px-4 py-2.5 text-xs font-semibold transition-colors duration-200 hover:bg-[#314536] active:scale-95" style={{ background: HOME_COLORS.primary, color: HOME_COLORS.onPrimary }}><Plus size={14} />Add notification</button>}
    </div>
    {!available ? <p className="mt-6 rounded-2xl border border-dashed p-5 text-sm" style={{ color: HOME_COLORS.onSurfaceVariant, borderColor: HOME_COLORS.outlineVariant + '77' }}>Workflow notifications will appear here once this workspace feature is activated.</p> : <>
