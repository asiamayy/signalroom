...
export function WorkspaceAutomations({ workspaceId }: { workspaceId: string }) {
  ...
  return <section className="mt-10 rounded-[2rem] border p-6 sm:p-8" style={{ background: HOME_COLORS.surfaceContainerLowest, borderColor: HOME_COLORS.outlineVariant + '55' }}>
    ...
    {open && <div className="mt-7 rounded-2xl border p-5 sm:p-6" style={{ background: HOME_COLORS.surfaceContainerLow, borderColor: HOME_COLORS.outlineVariant + '66' }}>
      <div className="flex gap-2">{(['slack', 'teams'] as WorkspaceWebhookProvider[]).map(item => <button key={item} type="button" onClick={() => setProvider(item)} className="rounded-full px-4 py-2 text-xs font-semibold" style={{ background: provider === item ? HOME_COLORS.primary : HOME_COLORS.surfaceContainerLowest, color: provider === item ? HOME_COLORS.onPrimary : HOME_COLORS.onSurfaceVariant }}>{item === 'slack' ? 'Slack' : 'Microsoft Teams'}</button>)}</div>
