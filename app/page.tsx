...
const DASHBOARD_PERSONAS: DashboardPersona[] = [
  ...
  },
  {
    ...
    imgUrl: '/landing-personas/adaeze.jpg',
    tags: ['SaaS', 'product marketing', 'go-to-market', 'frequent launcher', 'mid-career leader'],
    bio: 'Adaeze joined her current company 18 months ago from a mid-size fintech and is still building trust with a product org that historically viewed marketing as a support function rather than a strategic partner. She mentors two junior PMMs...',
    question: HERO_INTERVIEW_QUESTION,
    interviewQuote: "My team loses weeks waiting on research before every launch, and slow positioning work is exactly what gets bottlenecked. If this gave me a directionally honest read before messaging gets locked in, that's real time back — but I'd want to see it catch a positioning miss we already know happened, not just confirm what we'd have guessed anyway.",
    conviction: 71,
  },
  {
    id: 'meiling',
    ...
    imgUrl: '/landing-personas/meiling.jpg',
    tags: ['early-stage founder', 'B2B SaaS', 'pre-seed', 'solo operator mindset', 'Asian woman founder'],
    bio: "Mei-Ling spent 6 years as a PM at mid-size SaaS companies and watched founders make decisions she was convinced she could make better — so she finally bet on herself. She's self-funding the pre-seed stage with savings and a small friends-and-family round...",
    question: HERO_INTERVIEW_QUESTION,
    interviewQuote: "Every dollar I spend right now has to earn its place — a $15K research engagement isn't something a self-funded team of three can justify. If this gets me even a rough read on a decision in an afternoon, that's real leverage. I'd just want proof it catches something my own gut would've missed, not just echoes it back.",
    conviction: 76,
  }
];
    ...
    <div className="overflow-x-hidden relative min-h-screen bg-[#FCFCFB] text-[#121314] antialiased" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
            ...
            <div className="mt-14 grid grid-cols-1 gap-5 md:grid-cols-3">
              {DASHBOARD_PERSONAS.map((persona) => {
                ...
                return <button key={persona.id} onClick={() => setSelectedPersona(persona)} className={`group rounded-xl border p-5 text-left transition-all duration-500 ${active ? 'border-[#1A3024]/25 bg-white shadow-xl shadow-[#1A3024]/10 -translate-y-1' : 'border-[#d1d5d3] bg-[#fafbfa]/70 hover:border-[#aeb7af] hover:bg-white'}`}>
                  <div className={`relative mb-5 aspect-square overflow-hidden rounded-lg bg-[#edf0ec] transition-colors duration-500 ${active ? '' : 'group-hover:bg-[#e1e8e1]'}`}><img src={persona.imgUrl} alt={persona.name} className={`h-full w-full object-cover transition-all duration-700 ${active ? 'scale-100' : 'scale-95 saturate-[0.84] brightness-[1.02] group-hover:scale-100 group-hover:saturate-100'}`} />{active && <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/65 to-transparent p-3 text-[10px] font-medium text-white">{persona.location}</div>}<div className={`pointer-events-none absolute inset-0 rounded-lg border transition-colors duration-500 ${active ? 'border-transparent' : 'border-[#1A3024]/0 group-hover:border-[#1A3024]/10'}`} /></div>
      ...
      <RevealSection>
        <section id="features" className="relative border-b border-[#1A3024]/10 px-6 py-20 sm:px-12 sm:py-28">
          <div className="mx-auto max-w-6xl">
            <div className="mb-12 flex items-center gap-4"><span className="text-[10px] font-medium uppercase tracking-[0.4em] text-[#5A7973]">02 // Features</span><span className="hidden h-px flex-1 bg-[#1A3024]/10 sm:block" /></div>
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-12 grid grid-cols-1 gap-px overflow-hidden rounded-xl bg-[#d1d5d3]/30 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                { icon: 'center_focus_strong', title: 'Creative Testing', description: 'See where attention actually lands on your packaging, ad, or landing page — measured by a model trained on real eye-tracking research, not a guess — then hear how each persona reads what’s there.' },
