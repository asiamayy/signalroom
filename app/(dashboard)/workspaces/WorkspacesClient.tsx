...
function WorkspaceIntelligence({ workspaceId = 'workspace', personas, interviews, reports, activity = [], members = [] }: { workspaceId?: string; personas: Persona[]; interviews: (Interview & { persona: Persona })[]; reports: (Report & { interview: Interview })[]; activity?: WorkspaceActivity[]; members?: WorkspaceMember[] }) {
  ...
  return <section className="mb-9 border-t pt-8" style={{ borderColor: `${HOME_COLORS.outlineVariant}55` }}>
    ...
    <style>{`.workspace-intelligence > header nav { transform: scale(.84); transform-origin: right center; }`}</style>
    ...
    <div className={intelligenceTab === 'insights' ? 'block' : 'hidden'}>
...
function WorkspaceIntelligenceTabs({ active, onChange }: { active: 'analytics' | 'insights'; onChange: (tab: 'analytics' | 'insights') => void }) {
  return <nav className="flex rounded-full border border-white/30 bg-white/20 p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.55),0_10px_22px_-18px_rgba(0,0,0,0.75)] backdrop-blur-xl"><button type="button" onClick={() => onChange('analytics')} className={`rounded-full px-4 py-2 text-xs font-medium transition-colors ${active === 'analytics' ? 'bg-white text-[#18281C] shadow-[0_3px_10px_rgba(24,40,28,0.12)]' : 'text-white hover:bg-white/20'}`}>Analytics</button><button type="button" onClick={() => onChange('insights')} className={`rounded-full px-4 py-2 text-xs font-medium transition-colors ${active === 'insights' ? 'bg-white text-[#18281C] shadow-[0_3px_10px_rgba(24,40,28,0.12)]' : 'text-white hover:bg-white/20'}`}>Insight graph</button></nav>
}
