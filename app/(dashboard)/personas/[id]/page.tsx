    ...
    <div style={{ background: '#F4F6F8', minHeight: '100%' }}>
      ...
      <div className="p-6 pb-0">
        <div
          ...
          style={{ background: 'linear-gradient(115deg, #0A4F3A 0%, #147A5C 40%, #2BAE86 75%, #7DE0C0 100%)' }}
        >
          {/* Wave background */}
          <svg className="absolute inset-0 w-full h-full opacity-10" viewBox="0 0 800 200" preserveAspectRatio="none">
            <path d="M0,80 C200,160 400,0 600,100 C750,170 850,60 800,80 L800,200 L0,200Z" fill="white"/>
            <path d="M0,130 C150,60 350,190 550,110 C700,50 800,140 800,100 L800,200 L0,200Z" fill="white" opacity="0.5"/>
          </svg>

          {/* Floating stat cards — top right */}
          <div className="absolute top-5 right-6 flex gap-3 z-10">
            <div className="text-center px-4 py-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.2)' }}>
              <div className="text-xl font-bold text-white leading-none">{interviews?.length ?? 0}</div>
              <div className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.65)' }}>Interviews</div>
            </div>
            <div className="text-center px-4 py-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.2)' }}>
              <div className="text-xl font-bold text-white leading-none">Today</div>
              <div className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.65)' }}>Last active</div>
            </div>
          </div>

          {/* Main content */}
          <div className="relative z-10 flex items-center gap-5 px-7 pt-6 pb-5">
            ...
            {/* Start interview button — top right of content area */}
            <Link href={`/interviews/new?persona_id=${persona.id}`} className="flex-shrink-0 self-start mt-0">
              <button className="flex items-center gap-1.5 text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors" style={{ background: 'rgba(255,255,255,0.95)', color: '#0D5C45', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
          ...
          <div className="relative z-10 flex gap-8 px-7 py-4" style={{ background: 'rgba(0,0,0,0.15)', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
            <div>
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs font-semibold ml-1.5" style={{ color: 'rgba(255,255,255,0.8)' }}>{t?.tech_savviness ?? 0}/5</span>
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs font-semibold ml-1.5" style={{ color: 'rgba(255,255,255,0.8)' }}>{t?.risk_tolerance ?? 0}/5</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-5" style={{ background: 'white', boxShadow: '0 1px 3px rgba(0,0,0,0.06), 0 4px 16px rgba(0,0,0,0.05)', border: '1px solid rgba(0,0,0,0.04)' }}>
          ...
          {(!interviews || interviews.length === 0) ? (
            <div className="text-center py-4">
              ...
              <Link href={`/interviews/new?persona_id=${persona.id}`} className="text-xs font-medium" style={{ color: '#1A8C6A' }}>
            ...
            <div>
              {interviews.map((iv: Interview) => (
                ...
                >
                  ...
                  <ChevronRight size={12} className="text-neutral-300 flex-shrink-0 ml-2" />
                </Link>
              ))}
            </div>
          )}
