            ...
            {trendingSignals.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {trendingSignals.map(({ signal }) => {
                  ...
                  const evidenceSourceCount = sourceCount || signal.supporting_quotes.length
                  // A signal becomes validated at four supporting sources. Keep the
                  // visual scale aligned with that threshold rather than confidence.
                  const evidenceStrength = Math.min(evidenceSourceCount, 4) / 4 * 100
                  return (
                    <Link key={signal.id} href={`/projects/${signal.project_id}?tab=Signals`} className="rounded-2xl overflow-hidden block group transition-transform hover:-translate-y-0.5" style={{ background: HOME_COLORS.surfaceContainerLow, border: `1px solid ${HOME_COLORS.outlineVariant}33`, boxShadow: CARD_SHADOW }}>
                      <div className="flex h-36 flex-col justify-between p-5" style={{ background: `linear-gradient(135deg, ${HOME_COLORS.primaryContainer}, ${HOME_COLORS.primary})` }}>
                        <div className="flex items-start justify-between"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/10"><TypeIcon size={20} style={{ color: HOME_COLORS.primaryFixedDim }} strokeWidth={1.4} /></span><span className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/70">{signal.status}</span></div><div className="text-right"><p className="text-2xl leading-none" style={{ fontFamily: HOME_FONT_DISPLAY, color: HOME_COLORS.primaryFixed }}>{signal.confidence_score}%</p><p className="mt-1 text-[9px] font-bold uppercase tracking-wider text-white/55">Confidence</p></div></div>
                        <div><div className="mb-2 flex items-center justify-between text-[10px] text-white/60"><span>Evidence strength</span><span>{evidenceSourceCount >= 4 ? '4+ sources' : `${evidenceSourceCount} source${evidenceSourceCount === 1 ? '' : 's'}`}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full" style={{ width: `${evidenceStrength}%`, background: HOME_COLORS.primaryFixedDim }} /></div></div>
                      </div>
                      <div className="p-6">
